...
// Ne garder que les services tiktok
$services = array_filter($services_all, function($s) {
  return stripos($s['name'], 'tiktok') !== false;
});
?>
...
<body>
  ...
  <header>
    <div class="logo">
      <h1>SkyBoost</h1>
      <p>
        Boostez votre présence sur tiktok
      </p>
    </div>
  ...
  <div class="container">
    <h2>Commandes tiktok</h2>
    <form method="post" action="php/traitement_commande.php">
      <label for="service">Service tiktok :</label>
      <select name="service" id="service" required>
        <option value="">-- Sélectionner un service tiktok --</option>
  ...
  <script>
    $(document).ready(function () {
      $('#service').select2({
        placeholder: "Sélectionner un service tiktok",
        width: '100%'
      });
    });
