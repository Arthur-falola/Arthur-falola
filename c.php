...
// Ne garder que les services youtube
$services = array_filter($services_all, function($s) {
  return stripos($s['name'], 'youtube') !== false;
});
?>
...
<body>
  ...
  <header>
    <div class="logo">
      <h1>SkyBoost</h1>
      <p>
        Boostez votre présence sur youtube
      </p>
    </div>
  ...
  <div class="container">
    <h2>Commandes youtube</h2>
    <form method="post" action="php/traitement_commande.php">
      <label for="service">Service youtube :</label>
      <select name="service" id="service" required>
        <option value="">-- Sélectionner un service youtube --</option>
  ...
  <script>
    $(document).ready(function () {
      $('#service').select2({
        placeholder: "Sélectionner un service youtube",
        width: '100%'
      });
    });
    ...
    function updatePrice() {
      ...
      const total = (qty * selected.rate *600 / 1000).toFixed(4);
